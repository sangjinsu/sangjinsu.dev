...
미니멀하고 모던한 디자인에 3D 애니메이션을 활용하여 개발자로서의 정체성과 인생 스토리를 담습니다.

---

## 🎨 디자인 콘셉트: CORE / LATENT

> "겉으로는 조용하지만 내부에 잠재 에너지가 축적된 코어"
...
- 형태: 완전한 구가 아닌 비대칭, 미세하게 갈라진 레이어 구조
- 색상: 딥 네이비 ~ 차콜
- 질감: 매끈하지만 완전하지 않음 (미세한 노이즈)
- 항상 공중에 부유

#### 2. Inner Pulse (내부 맥동)

- 일정한 리듬으로 빛이 약하게 뛰는 파동
- 평소엔 거의 안 보임
- 마우스 호버/클릭 시 강해짐

#### 3. Outer Fragments (외곽 파편)

- 코어 주위를 천천히 도는 작은 조각들 (5-8개)
- 완전히 분리되지 않고 보이지 않는 힘으로 연결
- 각자 다른 궤도로 회전
...
| 상태     | 동작                          | 트리거      |
| -------- | ----------------------------- | ----------- |
| Idle     | 느린 회전, 파편 궤도 이동     | 기본        |
| Thinking | 회전 감속, 파편 수렴, 푸른 빛 | 마우스 호버 |
| Decision | 파편 정렬, 강한 빛, 방향 이동 | 클릭/스크롤 |
...
│   │   └── Contact.tsx     # 연락처
│   ├── three/
│   │   ├── CoreScene.tsx   # 3D 씬 컨테이너
│   │   ├── Core.tsx        # 메인 코어 오브젝트
│   │   ├── Fragments.tsx   # 외곽 파편들
│   │   └── Effects.tsx     # 포스트 프로세싱
│   └── ui/                 # shadcn/ui 컴포넌트
│       ├── button.tsx
│       ├── card.tsx
│       ├── badge.tsx
...
# 추가 패키지 설치 (Bun)
bun add @react-three/fiber @react-three/drei three framer-motion next-themes
...
### Phase 2: 3D CORE 구현 (`phase/2-3d-core`)

- [ ] React Three Fiber 설정
- [ ] Core 오브젝트 (비대칭 구체)
- [ ] Outer Fragments (외곽 파편)
- [ ] Inner Pulse (내부 맥동)
- [ ] 마우스 인터랙션 (호버/클릭)
...
- [ ] 페이지 진입 애니메이션
- [ ] 3D 상태 전환 (Idle → Thinking → Decision)
- [ ] 호버 효과
...
├── phase/2-3d-core     # Phase 2: CORE 3D 애니메이션
